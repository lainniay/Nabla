...
| P1 | `workspace.ts` | 删除 | path security 应归 filesystem permission |
...
Agent Profile 仍然可以维持当前对用户友好的配置格式，但加载后应立即编译。
...
如果 `policy/tool-policy.ts` 没有其他职责，则整个文件删除。
...
interface WorkspacePathResolver {
  ...
  canonicalFrom(root: string, path: string): string;
...
Plan Mode 不是 runtime infrastructure，应归：
...
# 14. Interaction：三个 Queue 重复实现生命周期
...
```text
PendingRequestRegistry<T>
```

但 Promise / Abort / lifecycle 仍在三套 Queue 中分别实现。

---

# 15. Interaction 应抽一个统一 primitive

推荐：

```ts
class InteractiveRequest<TRequest, TResponse> {
  request(...)
  resolve(...)
  reject(...)
  cancel(...)
...
# 16. `InteractionBroker` 可以删除

当前 `InteractionBroker` 本质只是：
...
```text
features/interactions/interaction-broker.ts
```

如果需要统一 lifecycle，只保留一个非常轻量的：
...
# 17. “ApprovalBroker” 命名应收敛

目前权限模块中还有：

```text
permissions/approvals/broker.ts
```
...
persist selected grant
```

它实际上不是 UI approval broker。
...
# 18. Permission Adapter 当前存在明显样板重复

当前 FileSystemAdapter 和 ShellAdapter 都存在类似：

```ts
proposeGrants(intent) {
...
这意味着 Adapter interface 承担了并不需要 adapter-specific 的职责。

---

## 18.1 Adapter 应缩成 normalize

推荐：

```ts
interface PermissionAdapter<T> {
  normalize(
    context: ToolContext,
    input: T
  ): PermissionIntent;
...
```text
proposeGrantBundles(intent)
explainPermissionIntent(intent)
```
...
```text
exactGrantProposals()
adapter.proposeGrants()
adapter.explain()
重复 default implementation
```
...
# 21. Permission execution broker 应改成 ExecutionPermit
...
如果仓库中 `ExecutionBroker.executeShell()` 没有其他真实调用，还可以进一步删除：
...
# 23. Context 建议删除薄 Service，拆真正的大实现
...
目前 `context-manager.ts` 中的职责大致可以分成：
...
应由 Controller / Host Event Adapter 负责。

不要因为发送 event 再包一层 Service。

---

# 24. `harness.ts` 最终应该消失
...
# 26. SubagentSupervisor 正在变成第二个 Composition Root
...
# 28. 主 Agent 与 Subagent 应共享 Permission Tool Guard
...
# 29. IntegrationService 与 WorktreeManager 应只保留一个公开 API
...
# 31. Sessions：目前是一个巨型实现 + 多个薄 Service
...
# 33. SessionService 与 RuntimeSupervisor 边界要明确
...
# 35. Protocol 推荐使用 schema-first

项目已有 TypeBox，可以让 schema 成为类型和 validator 的单一来源。
...
`HostEvent` 可以在 registry 组合，但不要让所有协议继续集中在一个巨大 `contracts.ts`。

---

# 37. Rust 协议 DTO 应逐步去掉裸 String
...
# 38. Rust / TypeScript DTO 不需要立刻强制 codegen
...
# 39. Rust：Selection 算法存在可直接删除的重复
...
# 41. Rust `file_references.rs` 与 `app/file_references.rs` 不应误合并
...
# 42. Rust `ui/text.rs` 应成为唯一文本布局 primitive
...
# 43. Rust Scene 应按 UI Surface 拆分
...
可以保持 reducer/effect 架构，但把 enum 按领域组合。
...
# 50. 最重要的一点：只保留一个 Permissions Root
...
以下几类虽然表面“重复”，但不建议粗暴合并。

---

## 52.1 Rust DTO 与 TypeScript DTO
...
## 52.2 Plan Tool Exposure 与 Permission Denial
...
## 52.3 Lexical Path Check 与 Realpath Check
...
## 52.4 Rust State 与 UI ViewModel

它们不属于错误重复。

Domain state 与 presentation state 应继续分离。

---

## 52.5 `file_references.rs` 与 `app/file_references.rs`
...
# Phase 6：统一 Interaction Lifecycle

目标：

```text
Approval / Question / Auth 共用 pending primitive
```
...
```text
InteractiveRequest<TReq, TRes>
```

2. Approval / Question / Auth 提供 policy。

3. 删除重复 Abort / Promise / Registry boilerplate。
...
5. 将 permission approval broker 重命名为：
...
# Phase 8：Sessions / Context / Worktree
...
## PR 8 — Sessions / Context / Worktree
...
```text
只有一个 pending request lifecycle implementation
```

Approval / Question / Auth 只定义差异策略。
...
## 57.8 Root-level historical modules
...
- [ ] `InteractionBroker` 是否已经删除？
- [ ] Permission 的 Grant Broker 是否已经改名为 GrantManager？
- [ ] PermissionAdapter 是否只负责 normalize？
- [ ] `ExecutionBroker` 是否仍有实际必要？
...
- [ ] `features/permissions` 与 `permissions` 是否已经合并？
- [ ] `policy/` 是否已经删除？
