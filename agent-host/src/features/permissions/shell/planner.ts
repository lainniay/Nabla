...
const ELEVATED_EXECUTABLES = new Set(["sudo", "su", "doas", "pkexec"]);
...
export function analyzeShell(
  ...
  const plan = planShell(source, cwd, environment);
  const elevated = plan.commands.some((command) =>
    ELEVATED_EXECUTABLES.has(executableName(command.executable)),
  );
  const destructive = plan.commands.some((command) =>
    isDangerousExecCommand(command.executable, command.argv, command.cwd),
  );
  const readOnly = plan.readOnly &&
    plan.commands.every((command) => isReadOnlyExec(command)) &&
    !destructive;
  return {
    plan,
    capabilities: plan.atoms,
    safety: {
      readOnly,
      mutating: !readOnly,
      network: plan.atoms.some((atom) => atom.kind === "network"),
      opaque: plan.opaque,
      elevated,
      destructive,
    },
...
function isReadOnlyExec(command: ExecCapability): boolean {
  const executable = executableName(command.executable);
  if (executable === "git") {
    return isReadOnlyGitCommand(command.argv, command.cwd);
  }
  if (executable === "cd") {
    return isReadOnlyCdCommand(command.argv, command.cwd);
  }
  if (executable === "find") {
    return isReadOnlyFindCommand(command.argv, command.cwd);
  }
  if (executable === "xargs") {
    return isReadOnlyXargsCommand(command.argv);
  }
  if (executable === "ls" || executable === "wc") {
    return isReadOnlyWorkspaceCommand(command.argv, command.cwd);
  }
  return isBenignShellCommand(executable);
}
